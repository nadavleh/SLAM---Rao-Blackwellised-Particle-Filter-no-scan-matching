...
Nm=10; %Number of global maps, each on with Np particels
threshold=0.75;
...
% with the according sumed particles weights.
    ...
    % the mean state of the robot (particles) within that map
...
% with no repeating arguments, arranged in acsending order.     
    ...
    map_mean_state = zeros(length(rsui),3);
    for i=1:length(rsui)
        ...
        map_mean_state(i,:)=mean(particles_state(:,:,rsui(i)),1);
